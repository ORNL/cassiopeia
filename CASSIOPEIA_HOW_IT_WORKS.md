...
**Full-text chunking for open-access papers.** For papers available in PubMed Central, the system goes one step further. Rather than indexing only the abstract, it fetches the complete article as structured XML (JATS format) and splits it into overlapping 400-token windows — one window per chunk, with an 80-token overlap so no sentence is cut off mid-thought at a boundary. Sections are parsed individually (introduction, methods, results, discussion), so the system knows *where* in a paper each chunk comes from. The entire set of chunks for a paper replaces the single abstract entry in the semantic index; retrieval then works at the chunk level and is deduplicated back to paper level before results are returned. For paywalled papers or sources where full text is unavailable, the abstract is kept as the indexed entry. The `full_text_indexed` flag on each paper record tracks which upgrade path was taken.
...
2. Retrieves the 12 papers most relevant to your profile by meaning (not just keywords). For papers indexed at chunk level (open-access PMC articles), up to 1,200 tokens of prioritised full-text content per paper is assembled — methods and results sections first, then discussion and introduction — giving the model a richer, section-aware view of each paper than the abstract alone. For paywalled papers where only the abstract was indexed, the abstract is used instead.
...
5. **Verifies the evidence behind each proposal.** Every key finding cited in a proposal's rationale — "paper X showed that …" — is checked back against the paper it claims to cite. A fast AI model reads the paper's text and judges whether the stated insight is genuinely supported by what the paper says. For open-access papers indexed at chunk level, verification reads up to 2,000 tokens drawn preferentially from the methods and results sections (where the primary evidence lives), rather than from the abstract alone. This makes verification more reliable: a claim about a protocol detail or a measured value is checked against the section of the paper where that detail would actually appear. The result is a small trust signal attached to every proposal: a count of how many cited claims were confirmed and how many were not. Proposals where a substantial fraction of claims could not be verified are flagged with a warning badge in the dashboard; all proposals show a "N out of M claims verified" note so you can assess the grounding at a glance. Nothing is hidden or dropped — you see every proposal alongside the evidence behind the evidence.
...
After the initial proposals are drafted, two further AI passes run automatically. The first checks that every finding cited in a proposal's rationale is genuinely supported by the paper it references — catching cases where a claim has been subtly overstated or attributed to the wrong source. For papers whose full text has been indexed at chunk level, the verifier reads up to 2,000 tokens assembled from the methods and results sections first (the sections where primary evidence typically appears), giving it a much richer view than an abstract alone. The second pass acts as a skeptical reviewer, evaluating each proposal for novelty, design confounds, and practical feasibility, and returning a concise recommendation alongside its reasoning. Together, these passes mean that by the time a proposal reaches you, it has been drafted, evidence-checked, and independently critiqued — not just generated.
